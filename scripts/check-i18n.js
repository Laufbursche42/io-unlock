...
// io-unlock ships a single script: app.js.
...
// Placeholder strings are wired through data-t-ph and applied by applyLang().
const attrPh = /data-t-ph="([^"]+)"/g;
while ((m = attrPh.exec(html)) !== null) usedHtml.add(m[1]);
...
// 'dp' is a label table indexed at run time by DP code (dpLabel(code)); its entries are not literal keys.
const dynamic = new Set(['phase', 'msg', 'dp']);
